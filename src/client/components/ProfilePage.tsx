...
export default async function ProfilePage({ username, sessionUsername, activeTab = "products" }: Props) {
            ...
            <div class="max-w-5xl mx-auto px-6">
                <div class="flex gap-2 ">
                    <button
                        ...
                        hx-swap="outerHTML"
                        hx-push-url="true"
                        class={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-all ${isProducts ? activeBtnClass : inactiveBtnClass}`}
                    >
                        {isOwnProfile ? "I miei prodotti" : `Prodotti di ${user.name}`}
                    </button>
                    {isOwnProfile && (
                        <>
                            <button
                                hx-get={`/profile?tab=orders`}
                                hx-target="#profile-wrapper"
                                hx-swap="outerHTML"
                                hx-push-url="true"
                                class={`px-4 py-2.5 text-sm -mb-px font-medium border-b-2 transition-all ${isOrders ? activeBtnClass : inactiveBtnClass}`}
                            >
                                I miei ordini
                            </button>
                            <button
                                hx-get={`/profile?tab=sold`}
                                hx-target="#profile-wrapper"
                                hx-swap="outerHTML"
                                hx-push-url="true"
                                class={`px-4 py-2.5 text-sm font-medium -mb-px border-b-2 transition-all ${isSold ? activeBtnClass : inactiveBtnClass}`}
                            >
                                Ordini ricevuti
                            </button>
                        </>
                            ...
                            <div class="divide-y divide-gray-100">
                                {visibleProducts.map(product => {
                                    ...
                                    if (product.imageUrl) {
                                        ...
                                        } catch { coverImage = product.imageUrl} 
                                    }
                                    ...
                                    return (
                                        <div
                                            ...
                                            class="flex items-center justify-between py-3 gap-4 product-item-row hover:bg-gray-50/80 px-2 -mx-2 rounded-xl cursor-pointer transition-colors"
                                        >
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <div class="flex items-center gap-2">
                                                    <h3 class="font-medium text-gray-800 truncate">{product.productName}</h3>
                                                    {isOwnProfile && (
                                                        <div class="relative group flex items-center cursor-help select-none shrink-0" onclick="event.stopPropagation()">
                            ...
                            <div class="divide-y divide-gray-100">
                                {userOrders.map(order => {
                                    ...
                                    if (order.product?.imageUrl) {
                                        ...
                                        } catch { orderCoverImage = order.product.imageUrl} 
                                    }
