...
function ExistingImageCard({ url, index, isCover }: { url: string; index: number; isCover: boolean }) {
  return (
    <div
      id={`existing-card-${index}`}
      class={`img-card preview-card relative border-2 rounded-xl overflow-hidden aspect-square bg-gray-50 flex flex-col justify-end p-2 transition-all shadow-xs group ${isCover ? "border-indigo-600 ring-2 ring-indigo-100" : "border-transparent"}`}
    >
      <input type="hidden" name="keepImages" value={url} id={`keep-${index}`} />
      <img src={url} class="absolute inset-0 w-full h-full object-cover z-0 pointer-events-none" />
      <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent z-10 pointer-events-none" />
      <button
        type="button"
        class="absolute top-1.5 right-1.5 z-30 bg-red-500 hover:bg-red-600 text-white rounded-full p-1 shadow-md transition-all opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer"
        onclick={`document.getElementById('keep-${index}').remove(); document.getElementById('existing-card-${index}').remove(); updateCoverIndices();`}
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3.5 h-3.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
        </svg>
      </button>
      <label class="relative z-20 bg-white/95 px-2 py-1 rounded-md text-[10px] font-bold text-gray-700 flex items-center gap-1.5 cursor-pointer shadow-xs mx-auto">
        <input
          type="radio"
          name="coverImage"
          value={url}
          checked={isCover}
          class="text-indigo-600 focus:ring-indigo-500 w-3 h-3 cursor-pointer"
          onchange="highlightCover(this.closest('.preview-card'))"
...
export default function EditProductModal({ product, error }: Props) {
  let existingImages: string[] = []
  try {
    existingImages = JSON.parse(product.imageUrl ?? "[]")
  } catch {
    existingImages = product.imageUrl ? [product.imageUrl] : []
  }
    ...
    <div id="modal-edit" class="fixed inset-0 bg-black/50 backdrop-blur-xs z-50 flex items-center justify-center p-4 transition-opacity animate-fade-in">
      ...
      >
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center shrink-0">
          <h2 class="text-xl font-bold text-gray-900">Modifica Prodotto</h2>
          <button
            type="button"
            onclick="document.getElementById('modal-edit').remove()"
        ...
        <form
          hx-post={`/edit-product/${product.id}`}
        ...
        >
          <div class="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div class="space-y-3 pt-2">
              <label class="block text-sm font-semibold text-gray-700 mb-1">Immagini Prodotto</label>
              <p class="text-xs text-gray-400">Rimuovi le immagini che non vuoi mantenere. Aggiungi nuove immagini con il pulsante.</p>

              <div class="grid grid-cols-3 gap-3">
                <div id="image-previews" class="contents">
                  {existingImages.map((url, i) => (
                    <ExistingImageCard url={url} index={i} isCover={i === 0} />
                  ))}
                </div>
                <button
                  type="button"
      ...
      <script type="text/javascript">
        {`
          ...
          window.highlightCover = function(selectedCard) {
            ...
            selectedCard.classList.add('border-indigo-600', 'ring-2', 'ring-indigo-100');
          };
          ...
          window.updateCoverIndices = function() {
            ...
            const hasChecked = [...cards].some(c => c.querySelector('input[type="radio"]')?.checked);
            if (!hasChecked) {
              const firstRadio = cards[0].querySelector('input[type="radio"]');
              if (firstRadio) { firstRadio.checked = true; highlightCover(cards[0]); }
            }
          };
          ...
          window.triggerImageUpload = function() {
            ...
            input.onchange = function() {
              ...
              reader.onload = function(e) {
                ...
                card.className = 'new-card preview-card relative border-2 rounded-xl overflow-hidden aspect-square bg-gray-50 flex flex-col justify-end p-2 cursor-pointer transition-all shadow-xs group border-transparent';
                ...
                delBtn.onclick = function(evt) { evt.stopPropagation(); card.remove(); input.remove(); updateCoverIndices(); };
                ...
                radio.type = 'radio';
                radio.name = 'coverImage';
                radio.value = 'new-' + currentId;
                radio.className = 'text-indigo-600 focus:ring-indigo-500 w-3 h-3 cursor-pointer';
                radio.onchange = function() { highlightCover(card); };
