...
export default function TransactionListModal({ soldOrders }: Props) {
  return (
    <div class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onclick="if(event.target===this)this.remove()">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6 relative">
        <button onclick="this.closest('.fixed').remove()" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 cursor-pointer">✕</button>
          ...
          <ul class="divide-y divide-gray-100">
            {soldOrders.map(order => (
              <li class="flex items-center justify-between py-3 gap-4">
                ...
                <span class="text-emerald-600 font-bold">+${(order.totalPrice ?? 0).toLocaleString("it-IT")}</span>
              </li>
            ))}
          </ul>
        )}
